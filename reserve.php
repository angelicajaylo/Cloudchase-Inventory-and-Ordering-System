...
<head>
  ...
  <title>CCVS | Reserve Orders</title>
  <?php 
  session_start();
  ...
  if(isset($_GET["remove"]))
    {
      $res_id=$_GET["remove"];
      $sqlremove = "UPDATE `reservation_tbl` SET `status` = 'Cancelled' WHERE `reservation_tbl`.`reservation_no` = $res_id;";
      $resultremove = mysqli_query($conn,$sqlremove);
    header("Location: reserve.php?result=Deleted");
    }
  ...
  $sqlmyres = "SELECT * FROM `reservation_tbl` where status = 'Reserved'";
  $resultmyres = mysqli_query($conn,$sqlmyres);
  $resultmyresad = mysqli_query($conn,$sqlmyres);
    
  $sqlusers = "SELECT * FROM `user_account`";
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
         ...
         <div class='col-sm-6'>
          ...
          <div class="col-sm-12 float-sm-right">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#"><i class="nav-icon fas fa-cart-arrow-down"></i> Reserve Orders</a></li>
    ...
    <section class="content">
      ...
      <div class="card">
              <div class="card-header ">
                <h3 class="card-title">List of Reserve Orders</h3>
              ...
              <div class="card-body">
                ...
                //     $price = number_format($row['price'], 2, '.', '');
                    ...
                    $sqlmyres = "SELECT * FROM `reservation_tbl` where status = 'Reserved'";
                    $resultmyresad = mysqli_query($conn,$sqlmyres);
                    while ($row1 = mysqli_fetch_array($resultmyresad))
                    {
                        if($row['user_id'] == $row1['customer_id']) {
                            $check = true; 
                            $t_amout +=  ($row1['price'] * $row1['qty']);
                        }
                    }
                    $t_amout = number_format($t_amout, 2, '.', '');
    ...
    <section class="content">
      ...
      <div class="card">
              <div class="card-header ">
                <h3 class="card-title">List of Your Reserve Orders</h3>
                <!-- <span class="float-sm-right"><button type="button" class="btn btn-success btn-block" data-toggle='modal' data-target='#modal-add'><i class="fa fa-plus"></i> Add New Hardwares</button></span> -->
              </div>
              <!-- /.card-header -->
              ...
              <div class="card-body">
                ...
                <?php
                if($resultCheck > 0 )
                while ($row = mysqli_fetch_array($resultmyres))
                {
                
                  $updatemod = '#update-'.$row['reservation_no'];
                  $deletemod = '#delete-'.$row['product_id'];
                    ...
                    <tr>
                        ...
                        <center>
                            ...
                            <a href='reserve.php?remove=$row[reservation_no]'><button type='submit' class='btn btn-danger'><i class='fa fa-trash'></i></button></a>
