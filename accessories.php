...
<head>
  ...
  $sql = "SELECT * FROM `product_tbl` where product_type = 'Accessory'";
  ...
  if(isset($_POST["addsubmit"]))
  {
    ...
    $sqladda = "INSERT INTO `product_tbl` (`product_id`, `name`, `product_type`, `juice_type`, `hardware_type`, `description`, `price`, `qty`, `status`) VALUES (NULL, '$aname', 'Accessory', '', '', '$adesc', '$aprice', '$aquantity', 'Available');";
    $resultadda = mysqli_query($conn,$sqladda);
    header("Location: accessories.php?result=Success");
  }
  ...
  { 
    ...
    $sqlupdatea = "UPDATE `product_tbl` SET `name` = '$aname', `description` = '$adesc', `price` = '$aprice', `qty` = '$aquantity' WHERE `product_tbl`.`product_id` = $aid;";
    $resultupdatea = mysqli_query($conn,$sqlupdatea);
    header("Location: accessories.php?result=Updated");
  }

  if(isset($_POST["submitdel"]))
  { 
    $aid=$_POST["aid"];
    $sqldelh = "DELETE FROM `product_tbl` WHERE `product_tbl`.`product_id` = $aid";
    $resultdelh = mysqli_query($conn,$sqldelh);
    header("Location: accessories.php?result=Deleted");
  }
  ...
  $sqlmodal = "SELECT * FROM `product_tbl` where product_type = 'Accessory'";
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="accessories.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Accessories</p>
                </a>
              </li>
      ...
      <div class="container-fluid">
        ...
        <?php
          ...
          if($notif=='Updated')
          {
            ...
            <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
            <h5><i class='icon fas fa-check'></i> Alert!</h5>
            Accessory Successfully Updated!
          </div>";
          }
          if($notif=='Deleted')
          {
    ...
    <section class="content">
      ...
      <div class="card">
              <div class="card-header">
                ...
                <span class="float-sm-right"><button type="button" class="btn btn-success btn-block" data-toggle='modal' data-target='#modal-add'><i class="fa fa-plus"></i> Add New Accessories</button></span>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title text-success"><i class='fas fa-info-circle'></i> View Accessory Info</h4>
              ...
              <div class='modal-dialog'>
                <div class='modal-content'>
                  <div class='modal-header  text-warning'>
                    <h4 class='modal-title'><i class='fas fa-edit'></i> Update Accessory Info </h4>
                    <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                      <span aria-hidden='true'>&times;</span>
                    </button>
                  </div>
                  <div class='modal-body'>
                    <form method='post'>
                    <div class='row'>
                      <label class='col-4'>Accessory Name:</label> 
                      <input type='text' class='form-control col-8  mb-2' hidden name='aid' value='$row[product_id]'>
                      <input type='text' class='form-control col-8  mb-2' name='aname' value='$row[name]'>
                      <label class='col-4'>Description:</label>
                      <textarea class='form-control col-8 mb-3' name='adesc'>$row[description]
                      </textarea>
                      <label class='col-4'>Accessory Price:</label> 
                      <input type='text' class='form-control col-8  mb-2' name='aprice' placeholder='&#8369 0.00' value='$row[price]'>
                      <label class='col-4'>Accessory Quantity:</label> 
                      <input type='number' class='form-control col-8  mb-2' name='aquantity' value='$row[qty]'>
                    </div>
                  </div>
                  <div class='modal-footer justify-content-between'>
                    <button type='button' class='btn btn-default' data-dismiss='modal'>Close</button> 
              ...
              <div class='modal-dialog'>
                <div class='modal-content'>
                  <div class='modal-header  text-danger'>
                    <h4 class='modal-title '><i class='fas fa-trash'></i> Delete Accessory Info</h4>
                    <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                      <span aria-hidden='true'>&times;</span>
                    </button>
                  </div>
                  ...
                  <form method='post'>
                  <input type='text' class='form-control col-8  mb-2' hidden name='aid' value='$row[product_id]'>
                    <p>Are you sure you want to DELETE this Accessories <span class='text-danger'>$row[name]</span>?</p>
                  </div>
                  <div class='modal-footer justify-content-between'>
                    <button type='button' class='btn btn-default' data-dismiss='modal'>Close</button>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header  text-success">
              <h4 class="modal-title "><i class='fas fa-plus'></i> Add New Accessory</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            ...
            <form method="post">
              <div class="row">
                <label class="col-4">Accessory Name:</label> 
                <input type="text" class="form-control col-8  mb-2" name='aname'>
                <label class="col-4">Description:</label>
                <textarea class="form-control col-8 mb-3" name="adesc">
                </textarea>
                <label class="col-4">Accessory Price:</label> 
                <input type="text" class="form-control col-8  mb-2" name='aprice' placeholder="&#8369 0.00">
                <label class="col-4">Accessory Quantity:</label> 
                <input type="number" class="form-control col-8  mb-2" name='aquantity'>
              </div>
            </div>
            <div class="modal-footer justify-content-between">
              <button type="button" class="btn btn-default" data-dismiss="modal">Close</button> 
